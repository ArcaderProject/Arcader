...
func _ready() -> void:
	...
	if Communicator.has_signal("connection_lost"):
		Communicator.connection_lost.connect(_on_api_connection_lost)
	if Communicator.has_signal("connection_restored"):
		Communicator.connection_restored.connect(_on_api_connection_restored)
	
	connection_lost.connect(_on_connection_lost)
...
func _on_api_connection_lost() -> void:
	last_connection_state = false
	emit_signal("connection_lost")

func _on_api_connection_restored() -> void:
